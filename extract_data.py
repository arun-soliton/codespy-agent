...
TARGET_PATH = r"D:\Source Codes\AI\knowledge-graph\cpp_calculator\src"  # CHANGE THIS
...
def analyze_single_file(
    ...
    def in_project(cursor: Cursor) -> bool:
        ...
        # Only consider definitions in the target file itself
        return file_path == normalize_path(cpp_file_path)
...
def analyze_directory(
    ...
    # Check for common include directory patterns
    include_candidate = directory_path / "include"
    if include_candidate.is_dir():
        include_dirs.append(normalize_path(include_candidate))
    ...
    def in_project(cursor: Cursor) -> bool:
        ...
        if location.file is None:
            return False
        file_path = normalize_path(Path(location.file.name))
        # Check if the file is one of our target cpp files
        return any(file_path == normalize_path(cpp_file) for cpp_file in cpp_files)
    ...
    def record_function(cursor: Cursor) -> Optional[str]:
        ...
        if usr not in function_data:
            ...
            function_data[usr] = {
                "qualified": qualified_name(cursor),
                "simple": cursor.spelling or cursor.displayname or "<anonymous>",
                "file": relative_to_project(file_path, directory_path),
                "line": cursor.location.line,
                "calls": set(),
                "description": extract_comment_text(cursor),
            }
